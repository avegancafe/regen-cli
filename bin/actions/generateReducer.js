...
function readOpts() {
  ...
  if (fs.existsSync("./package.json")) {
    ...
    if (fs.existsSync("./.regenrc.json")) {
      opts = require(path.resolve("./.regenrc.json"))
    ...
    reducersPath = basePath + "reducers/"
    actionsPath = basePath + "actions/"
    if (fs.existsSync(tmp + ".regenrc.json")) {
      opts = require(path.resolve(path.join(tmp + ".regenrc")))
      if (opts.reducersPath) {
...
function generateReducer() {
  var actionName = ARGS[2]
  var reducerName = ARGS[2]
  var name = ARGS[2]
  if (ARGS.length == 4) {
    actionName = ARGS[3]
  }
  ...
  if (fs.existsSync(path.join(actionsPath, name))) {
  ...
  fs.readFile(path.join(TEMPLATE_DIR, "reducer.js"), function (err, data) {
    fs.writeFileSync(
      path.join(reducersPath, reducerName + ".js"),
      data
        .toString()
  ...
  fs.readFile(path.join(TEMPLATE_DIR, "action.js"), function (err, data) {
    fs.writeFileSync(
      path.join(actionsPath, actionName + ".js"),
      data
        .toString()
        .replace(/{{reducer_name}}/g, objName)
        .replace(/{{reducer_name_caps}}/g, objName.toUpperCase())
  ...
  fs.readFile(path.join(reducersPath, "index.js"), function (err, data) {
    ...
    fin = fin.join("")
    var out = fin.slice(0, j) +
        "import " + camelCaseName + " from './" + name + "';\n" +
        fin.slice(j)
    fs.writeFileSync(
      path.join(reducersPath, "index.js"),
      out
    )
  })
}
